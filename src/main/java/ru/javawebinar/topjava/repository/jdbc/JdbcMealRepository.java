...
public class JdbcMealRepository implements MealRepository {
    ...
    @Autowired
    public JdbcMealRepository(JdbcTemplate template, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
        this.insertMeal = new SimpleJdbcInsert(template)
                .withTableName("meals")
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            Number insertKey = insertMeal.executeAndReturnKey(map);
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? AND date_time>=? " +
                "AND date_time<=?", MEAL_ROW_MAPPER, userId, startDateTime, endDateTime);
    }
}
